...
/**
 * @brief USB MIDI RX debug hook - unified implementation
 * 
 * Provides debug visibility of received USB MIDI packets while avoiding
 * stack overflow and timing issues from logging SysEx queries.
 * 
 * IMPORTANT: Skips SysEx packets (CIN 0x4-0x7) because:
 * - MIOS Studio queries are SysEx format
 * - Logging them causes stack overflow (~256 bytes per dbg_printf call)
 * - Creates timing delays that break query response processing
 * - Results in MIOS Studio timeout and recognition failure
 * 
