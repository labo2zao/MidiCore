...
static void *usb_cdc_switch_to_class_data(void) {
  void *class_data = USBD_COMPOSITE_GetClassData(&USBD_CDC);
  if (class_data == NULL) {
    return NULL;
  }

  return USBD_COMPOSITE_SwitchClassData(&hUsbDeviceFS, class_data);
}

static void usb_cdc_restore_class_data(void *previous) {
  if (previous != NULL) {
    (void)USBD_COMPOSITE_SwitchClassData(&hUsbDeviceFS, previous);
  }
}
...
int32_t usb_cdc_send(const uint8_t *buf, uint32_t len) {
  ...
  void *previous = usb_cdc_switch_to_class_data();
  if (previous == NULL) {
    return USB_CDC_NOT_READY;
  }
  ...
  usb_cdc_restore_class_data(previous);
...
uint8_t usb_cdc_is_connected(void) {
  ...
  uint8_t connected = USBD_CDC_IsConnected(&hUsbDeviceFS);
  usb_cdc_restore_class_data(previous);
  return connected;
}
